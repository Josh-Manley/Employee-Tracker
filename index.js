...
function addDepartment() {
  inquirer.prompt(addDepartmentQuestion).then((answer) => {
    db.query(`INSERT INTO department (name)
              VALUES ("${answer.departmentName}")`, function (err, results) {
      if (err) {
        console.log(err);
      } else {
  ...
  db.query('SELECT * FROM department', function (err, results) {
    if (err) {
      ...
      db.query(`INSERT INTO role (title, salary, department_id)
                VALUES ("${answer.roleName}", ${answer.salaryQuestion}, ${departmentId})`, function (err, results) {
        if (err) {
          console.log(err);
        } else {
              ...
              if (err) {
                ...
                db.query(`SELECT e.id, 
                  ...
                  if (err) {
                    ...
                    const selectedEmployee = results.find(employee => `${employee.first_name} ${employee.last_name}` === answer.employeeManager);
                    const managerId = selectedEmployee.id;

                    db.query(`INSERT INTO employee (first_name, last_name, role_id, manager_id)
                              VALUES ("${answer.employeeFirstName}", "${answer.employeeLastName}", ${roleId}, ${managerId})`, function (err, results) {
                      if (err) {
                        console.log(err);
                      } else {
